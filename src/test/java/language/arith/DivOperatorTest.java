...
//Test case that passes 1
	...
	public void testPerformOperationSetDiff() {
		operator.setOperand(0, new Operand<Integer>(15));
		operator.setOperand(1, new Operand<Integer>(20));

		Operand<Integer> result = operator.performOperation();
		int value = result.getValue();
		assertEquals("Operator applied to 15 and 20 should produce 0.", 0, value);
	}

	// Test case that passes 2
	@Test(timeout = 5000)
	public void testPerformOperationNegativeResultSetDiff() {
		operator.setOperand(0, new Operand<Integer>(-420));
		operator.setOperand(1, new Operand<Integer>(20));

		Operand<Integer> result = operator.performOperation();
		int value = result.getValue();
		assertEquals("Operator applied to -420 and 20 should produce -21.", -21, value);
	}

	// Test case that fails
	@Test(timeout = 5000, expected = IllegalStateException.class)
	public void testIllegalStateExceptionSetDiff() {
		operator.setOperand(0, new Operand<Integer>(25));
