...
public class PlusOperatorTest {
	...
	// Test case that passes number 1
	@Test(timeout = 5000)
	public void testPerformOperationSetDiff() {
		operator.setOperand(0, new Operand<Integer>(77));
		operator.setOperand(1, new Operand<Integer>(10));

		Operand<Integer> result = operator.performOperation();
		int value = result.getValue();
		assertEquals("Operator applied to 77 and 10 should produce 87.", 77 + 10, value);
	}

	// Test case that passes number 2
	@Test(timeout = 5000)
	public void testPerformOperationSetDiff2() {
		...
		Operand<Integer> result = operator.performOperation();
		int value = result.getValue();
		assertEquals("Operator applied to 100 and 1 should produce 87.", 100 + 1, value);
	}

	// Test case that fails
	@Test(timeout = 5000, expected = IllegalArgumentException.class)
	public void testIllegalArgumentExceptionSetDiff() {
		operator.setOperand(7, op0);
